...
export function PageShell({ children, isSessionActive }: PageShellProps): React.ReactElement {
  return (
    <div className={`dark w-full bg-neutral-950 text-neutral-100 flex flex-col items-center justify-center p-0 ${
      isSessionActive ? 'h-[100dvh] min-h-0 overflow-hidden' : 'min-h-screen overflow-x-hidden'
    }`}>
      {children}
    </div>
  );
