...
export function ActiveSessionPanel({
  ...
  return (
    <div className="w-full flex flex-col items-center justify-between h-full gap-5 animate-fade-in">
      ...
      <div className="flex flex-col items-center justify-center mt-2">
        ...
        >
          <span className="text-[11px] font-mono font-bold uppercase tracking-widest text-slate-350">
            {status}
          </span>
        </div>
      </div>
      ...
      <div className="w-full flex justify-center">
        {status === 'LISTENING' && (
          <div className="text-xs font-bold text-status-listening bg-status-listening/10 border border-status-listening/20 px-4 py-2 rounded-full flex items-center gap-2 animate-pulse font-mono uppercase tracking-wider shadow-inner">
            <Mic className="w-3.5 h-3.5 text-status-listening animate-bounce" />
            Your Turn: Speak now!
          </div>
        )}
        {status === 'SPEAKING' && (
          <div className="text-xs font-bold text-status-speaking bg-status-speaking/10 border border-status-speaking/20 px-4 py-2 rounded-full flex items-center gap-2 font-mono uppercase tracking-wider shadow-inner">
            <Volume2 className="w-3.5 h-3.5 text-status-speaking animate-pulse" />
            AI is Speaking...
          </div>
        )}
        {status === 'THINKING' && (
          <div className="text-xs font-bold text-status-thinking bg-status-thinking/10 border border-status-thinking/20 px-4 py-2 rounded-full flex items-center gap-2 font-mono uppercase tracking-wider shadow-inner">
            <Loader2 className="w-3.5 h-3.5 text-status-thinking animate-spin" />
            AI is Thinking...
          </div>
        )}
        {status === 'PROCESSING' && (
          <div className="text-xs font-bold text-status-processing bg-status-processing/10 border border-status-processing/20 px-4 py-2 rounded-full flex items-center gap-2 font-mono uppercase tracking-wider shadow-inner">
            <Loader2 className="w-3.5 h-3.5 text-status-processing animate-spin" />
            Processing Voice...
          </div>
        )}
        {status === 'IDLE' && (
